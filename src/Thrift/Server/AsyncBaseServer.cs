...
namespace Thrift.Server
{
    ...
    public class AsyncBaseServer : TBaseServer
    {
        ...
        public AsyncBaseServer(ITProcessorFactory itProcessorFactory,
            ...
            : base(
                itProcessorFactory, serverTransport, inputTransportFactory, outputTransportFactory,
        ...
        private async Task StartListening(CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    await
                        Task.Delay(TimeSpan.FromMilliseconds(_clientWaitingDelay), cancellationToken);
                }
            }
        ...
        private async Task Execute(TClientTransport client, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (ServerEventHandler != null)
                {
                    connectionContext =
                        await
                            ServerEventHandler.CreateContextAsync(inputProtocol, outputProtocol,
                                cancellationToken);
                }
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    {
                        await
                            ServerEventHandler.ProcessContextAsync(connectionContext, inputTransport,
                                cancellationToken);
                    }

                    if (
                        !await
                            processor.ProcessAsync(inputProtocol, outputProtocol, cancellationToken))
                    {
                        break;
                    }
                }
            ...
            {
                await
                    ServerEventHandler.DeleteContextAsync(connectionContext, inputProtocol,
                        outputProtocol, cancellationToken);
            }
