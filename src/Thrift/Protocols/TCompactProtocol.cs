    ...
    public class TCompactProtocol : TProtocol
    {
            ...
            await
                Trans.WriteAsync(
                    new[]
                    {
                        (byte)
                        ((Version & VersionMask) |
                         (((uint) message.Type << TypeShiftAmount) & TypeMask))
                    }, cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            */
            ...
            {
                await
                    Trans.WriteAsync(new[] {(byte) ((size << 4) | GetCompactType(elemType))},
                        cancellationToken);
            }
            else
            {
                await
                    Trans.WriteAsync(new[] {(byte) (0xf0 | GetCompactType(elemType))},
                        cancellationToken);
        ...
        public override async Task WriteBoolAsync(bool b, CancellationToken cancellationToken)
        {
            ...
            */
            ...
            if (_booleanField != null)
            {
                ...
                await
                    WriteFieldBeginInternalAsync(_booleanField.Value,
                        b ? Types.BooleanTrue : Types.BooleanFalse, cancellationToken);
                _booleanField = null;
            }
            else
            {
                ...
                await
                    Trans.WriteAsync(new[] {b ? Types.BooleanTrue : Types.BooleanFalse},
                        cancellationToken);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (protocolId != ProtocolId)
            {
                throw new TProtocolException(
                    $"Expected protocol id {ProtocolId:X} but got {protocolId:X}");
            }
        ...
        public override async Task<TMap> ReadMapBeginAsync(CancellationToken cancellationToken)
        {
            ...
            */
            ...
            var keyAndValueType = size == 0
                ? (byte) 0
                : (byte) await ReadByteAsync(cancellationToken);
